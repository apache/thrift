...
TJSONProtocol.prototype.readFieldBegin = function () {
  ...
  //get a fieldId
  // Own keys only, and the key that was found is the one deleted. for...in
  // also walks the prototype, and neither the lookup nor the delete below can
  // reach an inherited member -- so a field arriving that way was reported on
  // every call and never consumed, and the generated read() loop, which leaves
  // only on STOP, had nothing to end it.
  var struct = this.rstack[this.rstack.length - 1];
  var fnames =
    struct === null || typeof struct !== "object" ? [] : Object.keys(struct);
  ...
  if (fid != -1) {
    ...
    //way to match a key
    var typeSource = this.rstack[this.rstack.length - 1];
    var typeNames =
      typeSource === null || typeof typeSource !== "object"
        ? []
...
// The whole message is parsed before it is read, so the size a container
// declares can be checked against the elements it holds before reading them.
function checkContainerSize(size, available) {
  // No size at all has always read as an empty container, and still does.
  // readListEnd can leave an emptied list behind for the next readListBegin.
  if (size === undefined) {
    return;
  }
  if (!Number.isInteger(size)) {
    throw new Thrift.TProtocolException(
      ...
      "Container size is not an integer",
    );
  }
  if (size < 0) {
    throw new Thrift.TProtocolException(
...
TJSONProtocol.prototype.readListBegin = function () {
  ...
  r.size = list.shift();
  checkContainerSize(r.size, list.length);
