...
// Reads the 4-byte big-endian frame size from the front of the collected
// chunks. The size usually lives in the first chunk, but a packet boundary can
// split it across several.
function readFrameSize(chunks) {
  var head = chunks[0];
  if (head.length >= 4) {
    return binary.readI32(head, 0);
  }
  var sizeBytes = [];
  for (var i = 0; sizeBytes.length < 4 && i < chunks.length; i++) {
    var chunk = chunks[i];
    for (var j = 0; sizeBytes.length < 4 && j < chunk.length; j++) {
      sizeBytes.push(chunk[j]);
    }
  }
  return binary.readI32(sizeBytes, 0);
}

// maxLength is optional; when omitted no limit is enforced so that existing
// callers (e.g. clients reading responses) keep their previous behaviour.
TFramedTransport.receiver = function (callback, seqid, maxLength) {
  // Collect incoming chunks and only join them once a whole frame is present,
  // so accumulating a frame across many packets stays linear instead of
  // copying the entire residual on every packet.
  var chunks = [];
  var residualLength = 0;

  return function (data) {
    var chunk = Buffer.isBuffer(data) ? data : Buffer.from(data);
    chunks.push(chunk);
    residualLength += chunk.length;

    while (residualLength >= 4) {
      // Get single package size
      var frameSize = readFrameSize(chunks);
      if (maxLength && (frameSize < 0 || frameSize > maxLength)) {
        throw new Error(
          "Frame size " +
            frameSize +
      ...
      if (residualLength < 4 + frameSize) {
        return;
      }

      var residual =
        chunks.length === 1 ? chunks[0] : Buffer.concat(chunks, residualLength);
      // Get package data
      var frame = residual.subarray(4, 4 + frameSize);
      // Remove processed data from residual
      residual = residual.subarray(4 + frameSize);
      chunks = residual.length > 0 ? [residual] : [];
