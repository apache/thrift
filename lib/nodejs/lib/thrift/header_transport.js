...
// Makes sure the frame holds the fixed fields that end at the given offset.
var checkFrameLength = function (readBuffer, end) {
  if (readBuffer.length < end) {
    throw new THeaderTransportError("Frame is too small");
  }
};
...
THeaderTransport.prototype.readHeaders = function () {
  ...
  checkFrameLength(readBuffer, HEADER_MAGIC_OFFSET);
  ...
  checkFrameLength(readBuffer, FLAGS_OFFSET);
  ...
  checkFrameLength(readBuffer, HEADER_START_OFFSET);
  this.setFlags(readBuffer.readInt16BE(FLAGS_OFFSET));
  this.setSeqId(readBuffer.readInt32BE(SEQID_OFFSET));
  var headerSize = readBuffer.readUInt16BE(HEADER_SIZE_OFFSET) * 4;
