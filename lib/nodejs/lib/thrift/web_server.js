...
var wsFrame = {
  ...
  /** How many bytes the frame at the front of buf occupies.
   *
   * @param {Buffer} buf - Bytes received so far, starting at a frame boundary
   * @param {Number} maxLength - Largest frame we are willing to hold
   * @returns {Number} - The length of the first frame, or null if buf does not
   *                     contain all of it yet
   */
  frameLength: function (buf, maxLength) {
    if (buf.length < 2) {
      return null;
    }
    var lenByte = buf[1] & 0x0000007f;
    var len = lenByte;
    var dataOffset = 2;
    if (lenByte == 0x7e) {
      if (buf.length < 4) {
      ...
      // The length is 64 bits wide. Only the low half is usable here, so a
      // frame that sets the high half is one we would never accept anyway.
      if (buf.readUInt32BE(2) !== 0) {
        throw new Error("Invalid WebSocket frame: payload length too large");
      }
      len = buf.readUInt32BE(6);
      dataOffset = 10;
    }
    if (wsFrame.mask.TO_SERVER == (buf[1] & wsFrame.mask.TO_SERVER)) {
      dataOffset += 4;
    }
    // Decide on the declared length before waiting for it to arrive, so that
    // a frame nobody would accept costs nothing to refuse.
    if (dataOffset + len > maxLength) {
      throw new Error(
        "WebSocket frame of " +
          (dataOffset + len) +
    ...
    return buf.length < dataOffset + len ? null : dataOffset + len;
  },
  ...
  decode: function (frame) {
    ...
    if (lenByte == 0x7e) {
      ...
      // The length is 64 bits wide. Only the low half is usable here, so a
      // frame that sets the high half is one we could not have received in
      // full anyway.
      if (frame.readUInt32BE(2) !== 0) {
        throw new Error("Invalid WebSocket frame: payload length too large");
      }
    ...
    if (wsFrame.mask.TO_SERVER == (frame[1] & wsFrame.mask.TO_SERVER)) {
      if (dataOffset + 4 > frame.length) {
        throw new Error("Invalid WebSocket frame: mask is not present");
      }
    ...
    //Payload
    // The declared length has to describe bytes that are actually here. It is
    // what sizes the allocation below, and Buffer.copy would quietly truncate
    // rather than report a frame that promises more than it carries.
    if (dataOffset + len > frame.length) {
      throw new Error(
        "Invalid WebSocket frame: declared payload length " +
          len +
          " exceeds the " +
          (frame.length - dataOffset) +
          " bytes received",
      );
    }
...
exports.createWebServer = function (options) {
  ...
  function processPost(request, response) {
    ...
    //Process XHR payload
    var xhrReceiver = svc.transport.receiver(
      ...
      undefined,
      svc.transport.DEFAULT_MAX_LENGTH,
    );
  ...
  function processWS(data, socket, svc, binEncoding) {
    var wsReceiver = svc.transport.receiver(
      function (transportWithData) {
        var input = new svc.protocol(transportWithData);
        var output = new svc.protocol(
          new svc.transport(undefined, function (buf) {
  ...
  server
    ...
    .on("upgrade", function (request, socket, head) {
      //An error on this connection ends the connection, not the server
      ...
      //Lookup service
      var svc = services ? services[Object.keys(services)[0]] : undefined;
      if (!svc) {
      ...
      var data = null;
      var pending = null;
      var maxMessageLength = svc.transport.DEFAULT_MAX_LENGTH;
      socket.on("data", function (chunk) {
        try {
          //A frame can span several reads, so carry over whatever did not
          //amount to a whole one instead of decoding a partial frame.
          ...
          while (buffered.length > 0) {
            var frameLength = wsFrame.frameLength(buffered, maxMessageLength);
            if (frameLength === null) {
              pending = buffered;
              break;
            }
            var frame = buffered.subarray(0, frameLength);
            ...
            if (data) {
              if (result.data) {
                // Fragments of a message that never ends are still fragments
                // of one message, so hold their total to what the transport
                // would accept for a whole one.
                if (data.length + result.data.length > maxMessageLength) {
                  throw new Error(
                    "WebSocket message exceeds the maximum allowed " +
                      maxMessageLength +
                      " bytes",
                  );
                }
