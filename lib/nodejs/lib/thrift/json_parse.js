    ...
    object = function () {
      ...
      if (ch === "{") {
        ...
        while (ch) {
          ...
          var member = value();
          if (key === "__proto__") {
            // Plain assignment to "__proto__" runs the setter inherited from
            // Object.prototype, which swaps the object's prototype instead of
            // adding a member: the pair is lost, every name on the value shows
            // up in for...in, and the duplicate-key guard above never sees it
            // because it is not an own key. defineProperty always creates the
            // own data property, which is what JSON.parse produces.
