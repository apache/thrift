...
exports.createMultiplexServer = function (processor, options) {
  ...
  var maxLength =
    options && options.maxLength != null
      ? options.maxLength
      : transport.DEFAULT_MAX_LENGTH;

  function serverImpl(stream) {
    var self = this;
    // An error on this connection goes to the server's "error" listeners,
    // or to the log when there are none.
  ...
  if (options && options.tls) {
    // Leave an unset secureProtocol/secureOptions alone so Node applies its own
    // default minimum TLS version rather than re-enabling older protocols.
