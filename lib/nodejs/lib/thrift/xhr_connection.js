...
// flush() receives the replies of the binary protocols as an ArrayBuffer,
// and those of TJSONProtocol as a string.
function isArrayBuffer(buf) {
  return Object.prototype.toString.call(buf) == "[object ArrayBuffer]";
}
 ...
 * @param {string|ArrayBuffer} buf - The buffer to supply the protocol.
 */
XHRConnection.prototype.setRecvBuffer = function (buf) {
  this.recv_buf = buf;
  // An ArrayBuffer has a byteLength, but no length.
  this.recv_buf_sz = isArrayBuffer(buf) ? buf.byteLength : buf.length;
 ...
 * buffer, or the specified number of bytes if the response is binary.
 * @param {number} len - The number of characters or bytes to return.
 * @returns {string|Buffer} Characters sent by the server, or a Buffer
 *     if the response is an ArrayBuffer.
 */
XHRConnection.prototype.read = function (len) {
  var avail = this.wpos - this.rpos;

  if (avail === 0) {
    return isArrayBuffer(this.recv_buf) ? Buffer.alloc(0) : "";
  }
  ...
  var ret = isArrayBuffer(this.recv_buf)
    ? Buffer.from(new Uint8Array(this.recv_buf, this.rpos, give))
    : this.recv_buf.substr(this.rpos, give);
  this.rpos += give;
 ...
 * @returns {string|ArrayBuffer} Characters sent by the server, or an
 *     ArrayBuffer if the response is binary.
 */
