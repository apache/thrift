...
const cases = {
  ...
  "Should read I64": function(assert) {
    assert.equal("0", binary.readI64([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).toString());
    assert.equal("1", binary.readI64([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]).toString());
    assert.equal("-1", binary.readI64([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]).toString());

    // Min I64
    assert.equal(Number.MIN_SAFE_INTEGER, binary.readI64([0xff, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]).toNumber());
    assert.equal("-9223372036854775808", binary.readI64([0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).toString());
    // Max I64
    assert.equal(Number.MAX_SAFE_INTEGER, binary.readI64([0x1f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]).toNumber());
    assert.equal("9223372036854775807", binary.readI64([0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]).toString());
    assert.end();
  },

  "Should write I64": function(assert) {
    assert.deepEqual([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], binary.writeI64([], 0));
    assert.deepEqual([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], binary.writeI64([], 1));
    assert.deepEqual([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], binary.writeI64([], -1));
    assert.deepEqual([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], binary.writeI64([], "0"));
    assert.deepEqual([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], binary.writeI64([], "1"));
    assert.deepEqual([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], binary.writeI64([], "-1"));

    // Min I64
    assert.deepEqual([0xff, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], binary.writeI64([], Number.MIN_SAFE_INTEGER));
    assert.deepEqual([0xff, 0xe0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01], binary.writeI64([], Number.MIN_SAFE_INTEGER + ""));
    assert.deepEqual([0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], binary.writeI64([], "-9223372036854775808"));
    // Max I64
    assert.deepEqual([0x1f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], binary.writeI64([], Number.MAX_SAFE_INTEGER));
    assert.deepEqual([0x1f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], binary.writeI64([], Number.MAX_SAFE_INTEGER + ""));
    assert.deepEqual([0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], binary.writeI64([], "9223372036854775807"));
    assert.end();
  },
