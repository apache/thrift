...
// length does not describe the bytes that actually arrived, and what the
...
// The client's half of the WebSocket handshake.
...
// Sends a handshake to a server built from options, and expects it to be
// refused and the connection closed.
function expectRefusedUpgrade(options, assert) {
  const server = thrift.createWebServer(options);

  server.listen(0, "127.0.0.1", function () {
    let response = "";
    const socket = net.connect(server.address().port, "127.0.0.1", function () {
      socket.write(handshakeRequest());
    });
    const finish = once(server, socket, assert);
    ...
    socket.on("close", function () {
      ...
      finish(
        status === "HTTP/1.1 403 No Apache Thrift Service available",
        "the upgrade was refused and the connection closed (status line: " +
...
test("an upgrade is refused when there are no services", function (assert) {
  expectRefusedUpgrade({}, assert);
});

test("an upgrade is refused when the services object is empty", function (assert) {
  // Unlike a missing services object, an empty one does not make the service
  // lookup throw; it finds nothing.
  expectRefusedUpgrade({ services: {} }, assert);
});
...
test("an error on an upgraded connection ends only that connection", function (assert) {
  // With no "error" listener of its own, the upgraded socket's error would
  // escape as an uncaught exception and end the process.
  let uncaught = null;
  const onUncaught = function (err) {
    uncaught = err;
  };
  ...
  startServer(function (server, port) {
    let client = null;
    // Registered after the web server's own listener, so by the time this
    // runs the connection has been upgraded.
    server.once("upgrade", function (request, serverSide) {
      setImmediate(function () {
        serverSide.destroy(new Error("connection failed"));
        setImmediate(function () {
          http.get(`http://127.0.0.1:${port}/`, function (res) {
            res.resume();
            process.removeListener("uncaughtException", onUncaught);
            assert.equal(uncaught, null, "the error did not escape");
            assert.ok(serverSide.destroyed, "the connection was closed");
            assert.ok(res.statusCode > 0, "the server still answers requests");
            client.destroy();
            server.close();
            assert.end();
          });
