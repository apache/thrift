...
var TFramedTransport = require('../lib/thrift/framed_transport');
var THeaderTransport = require('../lib/thrift/header_transport');
var THeaderProtocol = require('../lib/thrift/header_protocol');
var thrift = require('../lib/thrift');
var fs = require('fs');
var test = require('tape');
var path = require('path');

var headerPayload = fs.readFileSync(path.join(__dirname, 'test_header_payload'));

var cases = {
  'Should read headers from payload': function(assert) {
    var transport = new TFramedTransport();
    transport.inBuf = Buffer.from(headerPayload);

    var headers = transport.readHeaders();
  ...
  'Should read headers when reading message begin': function(assert) {
    var transport = new TFramedTransport();
    transport.inBuf = Buffer.from(headerPayload);
    var protocol = new THeaderProtocol(transport);
    var result = protocol.readMessageBegin();

    var headers = transport.getReadHeaders();
    assert.equals(headers.Parent, 'shoobar');
    assert.equals(headers.Trace, 'abcde');
  ...
  'Should be able to write headers': function(assert) {
    var writeTransport = new TFramedTransport();
    ...
    var writeBuffer = writeTransport.outBuffers[0];

    var readTransport = new TFramedTransport();
    readTransport.inBuf = writeBuffer;
    readTransport.readHeaders();

    var headers = readTransport.getReadHeaders();
