...
type
  ...
  public type
    TExceptionType = (Unknown, NotOpen, AlreadyOpen, TimedOut, EndOfFile);
  private
  ...
  public
    constructor Create(const ATcpClient: TIdTCPClient);
  end;
  ...
  public
    constructor Create(const AServer: TIdTCPServer); overload;
    constructor Create(const AServer: TIdTCPServer; AClientTimeout: Integer); overload;
    constructor Create(APort: Integer); overload;
    constructor Create(APort: Integer; AClientTimeout: Integer); overload;
    constructor Create(APort: Integer; AClientTimeout: Integer; AUseBufferedSockets: Boolean); overload;
    destructor Destroy; override;
  ...
  public
    procedure Open; override;
    constructor Create(const AClient: TIdTCPClient); overload;
    ...
    destructor Destroy; override;
    procedure Close; override;
    property TcpClient: TIdTCPClient read FClient;
  ...
  private
    class var FHeader_Dummy: array of Byte;
  protected
...
function THTTPClientImpl.CreateRequest: TIdHTTP;
var
  IdHTTP: TIdHTTP;
...
//IdHTTP.HTTPOptions:= IdHTTP.HTTPOptions + [hoKeepOrigProtocol];
    ...
    IdHTTP.Request.UserAgent := 'Delphi/IHTTPClient';
    IdHTTP.Request.Accept := 'application/x-thrift';
    IdHTTP.Request.ContentType := 'application/x-thrift';
    for pair in FCustomHeaders do
    begin
      Result.Request.CustomHeaders.AddValue(pair.Key, pair.Value);
    end;
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      raise TTransportException.Create(TTransportException.TExceptionType.Unknown, E.Message);
    end;
  end;
end;
...
begin
  ...
  try
    ...
    ms.Position := 0;
//    s:= xmlhttp.Post(FUri, ms); // send(IUnknown(TStreamAdapter.Create(ms, soReference)));
    xmlhttp.Post(FUri, ms, response); // send(IUnknown(TStreamAdapter.Create(ms, soReference)));
    FInputStream := nil;

//    len := Length(s);
//    response.WriteBuffer(Pointer(@s)^, len);
response.Position:= 0;
    FInputStream :=  TThriftStreamAdapterDelphi.Create(response, false);
  finally
    ms.Free;
  end;
end;
...
constructor TTransportException.Create(AType: TExceptionType; const msg: string);
begin
...
constructor TServerSocketImpl.Create(const AServer: TIdTcpServer; AClientTimeout: Integer);
begin
  inherited Create;
  FServer := AServer;
...
constructor TServerSocketImpl.Create(const AServer: TIdTcpServer);
begin
  // no inherited;
  Create(AServer, 0);
end;
...
begin
  if FServer = nil then
  begin
    raise TTransportException.Create(TTransportException.TExceptionType.NotOpen, 'No underlying server socket.');
  end;
...
constructor TSocketImpl.Create(const AClient: TIdTcpClient);
var
...
begin
  if IsOpen then
  begin
    raise TTransportException.Create(TTransportException.TExceptionType.AlreadyOpen, 'Socket already connected');
  end;

  if FHost = '' then
  begin
    raise TTransportException.Create(TTransportException.TExceptionType.NotOpen, 'Cannot open null host');
  end;

  if Port <= 0 then
  begin
    raise TTransportException.Create(TTransportException.TExceptionType.NotOpen, 'Cannot open without port');
  end;
...
begin
  Result := (FWriteBuffer <> nil) and (FReadBuffer <> nil) and (FStream <> nil);
end;
...
begin
  ...
  FillChar(TFramedTransportImpl.FHeader_Dummy[0], Length(TFramedTransportImpl.FHeader_Dummy) * SizeOf(Byte), 0);
end;
{$ELSE}
...
begin
  ...
  begin
    if len > 0 then
      got := FReadBuffer.Read(Pointer(@buf[off])^, len)
    else
      got := 0;
    if got > 0 then
    begin
  ...
  ReadFrame;
  if len > 0 then
    Result := FReadBuffer.Read(Pointer(@buf[off])^, len)
  else
    Result := 0;
end;
...
begin
  ...
  Size := ((i32rd[0] and $FF) shl 24) or ((i32rd[1] and $FF) shl 16) or ((i32rd[2] and $FF) shl 8) or (i32rd[3] and $FF);
  SetLength(buff, Size);
...
begin
  if len > 0 then
    FWriteBuffer.Write(Pointer(@buf[off])^, len);
end;
...
constructor TTcpSocketStreamImpl.Create(const ATcpClient: TIdTcpClient);
begin
  inherited Create;
...
begin
  inherited;
  Result := 0; // FTcpClient. ReceiveBuf(Pointer(@buffer[offset])^, count);
end;
