...
pub trait TOutputProtocol {
    /// Write marker identifying the beginning of a Thrift message.
    /// This marker may contain any or all of the fields in `identifier`.
    fn write_message_begin(&mut self, identifier: &TMessageIdentifier) -> ::Result<()>;
    /// Write a marker identifying the end of a Thrift message.
    fn write_message_end(&mut self) -> ::Result<()>;
    fn write_struct_begin(&mut self, identifier: &TStructIdentifier) -> ::Result<()>;
    fn write_struct_end(&mut self) -> ::Result<()>;
    fn write_field_begin(&mut self, identifier: &TFieldIdentifier) -> ::Result<()>;
...
pub trait TInputProtocolFactory {
    fn build(&self, transport: Rc<RefCell<Box<TTransport>>>) -> Box<TInputProtocol>;
}

pub trait TOutputProtocolFactory {
    fn build(&self, transport: Rc<RefCell<Box<TTransport>>>) -> Box<TOutputProtocol>;
}
