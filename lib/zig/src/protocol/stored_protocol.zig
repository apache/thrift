...
pub const StoredProtocolError = error{
    MessageAlreadyRead,
};

pub const TStoredProtocol = struct {
    const Self = @This();

    allocator: Allocator,
    wrapped: TProtocol,
    storedMessage: TMessageIdentifier,
    messageConsumed: bool = false,

    pub fn init(allocator: Allocator, wrapped: TProtocol, message: TMessageIdentifier) !Self {
        return .{
            .allocator = allocator,
            .wrapped = wrapped,
            .storedMessage = .{
                .allocator = allocator,
                .name = try allocator.dupe(u8, message.name),
    ...
    pub fn readMessageBegin(self: *Self, allocator: Allocator) !TMessageIdentifier {
        if (self.messageConsumed) return StoredProtocolError.MessageAlreadyRead;
        self.messageConsumed = true;
        return .{
            .allocator = allocator,
            .name = try allocator.dupe(u8, self.storedMessage.name),
            .msgType = self.storedMessage.msgType,
            .sequenceNumber = self.storedMessage.sequenceNumber,
        };
    }
