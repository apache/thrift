...
Thrift.Transport = Thrift.TXHRTransport = function(url, options) {
    ...
    this.send_buf = '';
    this.recv_buf = '';
    this.$http = (options && options.$http);
};
...
Thrift.TXHRTransport.prototype = {
    ...
    /**
     * Creates an object using the provided $http service provided with the Transport config to be used for a Thrift server call.
     * @param {object} client - The Thrift Service client object generated by the IDL compiler.
     * @param {object} postData - The message to send to the server.
     * @param {function} args - The original call arguments with the success call back at the end.
     * @param {function} recv_method - The Thrift Service Client receive method for the call.
     * @returns {object} A promise provided by the $http service.
     * @throws {string} If the jQuery version is prior to 1.5 or if jQuery is not found.
     */
    ngRequest: function(client, postData, args, recv_method) {
        if (typeof this.$http === 'undefined' ) {
            console.log('Thrift.js requires the Angular $http service to be provided with Transport.options.$http field. Falling back to jqRequest.');
            return this.jqRequest(client, postData, args, recv_method);
        }

        var thriftTransport = this;

        var serviceUrl = this.url;
        var ngRequest = this.$http({
            method: 'POST',
            url : serviceUrl,
            data : postData,
            cache : false,
            ...
            transformResponse : function(responseData, headersGetter, status) {
                thriftTransport.setRecvBuffer(responseData);
                var value = recv_method.call(client);
                return value;
            }
        }).success(jQuery.makeArray(args).pop()); // TODO: jQuery always a dep of Angular?
