...
Thrift.Protocol.prototype = {
    ...
    readFieldBegin: function() {
        ...
        //get a fieldId
        // Own keys only, and the key that was found is the one deleted:
        // for...in also walks the prototype, and neither the lookup nor the
        // delete below can reach an inherited member, so such a field would be
        // reported on every call and never consumed.
        var struct = this.rstack[this.rstack.length - 1];
        var fnames = (struct === null || typeof struct !== 'object') ? [] : Object.keys(struct);
        ...
        if (fid != -1) {
            ...
            var typeNames = (typeSource === null || typeof typeSource !== 'object') ? [] : Object.keys(typeSource);
    ...
    readMapBegin: function() {
        ...
        r.size = map.shift();
        if (r.size < 0) {
            throw new Thrift.TProtocolException(Thrift.TProtocolExceptionType.NEGATIVE_SIZE, 'Negative size');
        }
        this.rpos.push(this.rstack.length);
