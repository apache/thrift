...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    shell: {
      ...
      ThriftGenNG: {
        command: 'thrift -gen js:angular=ngthriftApp -gen js:node -o test ../../test/ThriftTest.thrift'
      }
    },
    external_daemon: {
      ThriftTestServer: {
    ...
    qunit: {
      ...
      ThriftJSNG: {
        options: {
          urls: [
            'http://localhost:8088/test-angular.html'
          ]
        }
      },
      ThriftWS: {
      ...
      ThriftJSNG_TLS: {
        options: {
          '--ignore-ssl-errors': true,
          urls: [
            'https://localhost:8089/test-angular.html'
          ]
        }
      },
      ThriftWS_TLS: {
  ...
  grunt.registerTask('test', ['jshint', 'shell:InstallThriftJS', 'shell:InstallThriftNodeJSDep', 'shell:ThriftGen',
                              ...
                              'shell:ThriftGenNG', 'qunit:ThriftJSNG', 'qunit:ThriftJSNG_TLS'
                             ]);
  grunt.registerTask('default', ['jshint', 'shell:InstallThriftJS', 'shell:InstallThriftNodeJSDep', 'shell:ThriftGen',
                                 'external_daemon:ThriftTestServer', 'external_daemon:ThriftTestServer_TLS',
                                 'qunit:ThriftJS', 'qunit:ThriftJS_TLS',
                                 'shell:ThriftGenJQ', 'qunit:ThriftJSJQ', 'qunit:ThriftJSJQ_TLS',
                                 'shell:ThriftGenNG', 'qunit:ThriftJSNG', 'qunit:ThriftJSNG_TLS', // does this still make sense?
                                 'concat', 'uglify', 'jsdoc'
                                ]);
};
