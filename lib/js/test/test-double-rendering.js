...
QUnit.test("Double (rendering)", function(assert) {
  ...
  var EXPECTED_DOUBLE_ASSIGNED_TO_LARGE_DOUBLE = 1.7e308;
  var EXPECTED_DOUBLE_ASSIGNED_TO_LARGE_FRACTIONAL_DOUBLE = 9223372036854775816.43;
  var EXPECTED_DOUBLE_ASSIGNED_TO_SMALL_DOUBLE = -1.7e308;
  ...
  assert.equal(
    typeof DOUBLE_ASSIGNED_TO_DOUBLE_WITH_MANY_DECIMALS_TEST,
    "number"
  );
  assert.equal(typeof DOUBLE_ASSIGNED_TO_FRACTIONAL_DOUBLE_TEST, "number");
  assert.equal(
    typeof DOUBLE_ASSIGNED_TO_NEGATIVE_FRACTIONAL_DOUBLE_TEST,
    "number"
  );
  assert.equal(typeof DOUBLE_ASSIGNED_TO_LARGE_DOUBLE_TEST, "number");
  assert.equal(
    typeof DOUBLE_ASSIGNED_TO_LARGE_FRACTIONAL_DOUBLE_TEST,
    "number"
  );
  assert.equal(typeof DOUBLE_ASSIGNED_TO_SMALL_DOUBLE_TEST, "number");
  assert.equal(
    typeof DOUBLE_ASSIGNED_TO_NEGATIVE_BUT_LARGE_FRACTIONAL_DOUBLE_TEST,
    "number"
  );
  ...
  for (var i = 0; i < EXPECTED_DOUBLE_LIST.length; ++i) {
    assert.ok(
      Math.abs(EXPECTED_DOUBLE_LIST[i] - DOUBLE_LIST_TEST[i]) <= EPSILON
    );
  }
