...
QUnit.module('Transports');

['TXHRTransport', 'TWebSocketTransport'].forEach(function(name) {
  QUnit.test(name + ' reads from the receive buffer', function(assert) {
    const transport = new Thrift[name]();
