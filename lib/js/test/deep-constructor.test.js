...
function createThriftObj() {
  return new Complex({
    ...
    struct_list_field: [new Simple({ value: "b" }), new Simple({ value: "c" })],

    struct_set_field: [new Simple({ value: "d" }), new Simple({ value: "e" })],
    ...
    struct_nested_containers_field: [
      [
        {
          C: [new Simple({ value: "h" }), new Simple({ value: "i" })]
        }
      ]
    ],
    ...
    list_of_list_field: [["one", "two"], ["three", "four"], ["five", "six"]]
  });
}
...
var cases = {
  "Serialize/deserialize simple struct should return equal object": function(
    assert
  ) {
