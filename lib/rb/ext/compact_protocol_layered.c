...
static VALUE rb_read_set_begin(VALUE self) {
  DEBUG_FUNCTION_ENTRY();
  return rb_read_list_begin(self);
}
...
static VALUE rb_read_bool(VALUE self) {
  ...
  if (cd->current_field_active == 0) {
    return transfer_read_byte(cd) == CTYPE_BOOLEAN_TRUE ? Qtrue : Qfalse;
  } else {
    cd->current_field_active = 0;
    return bool_value;
  }
...
static VALUE rb_read_i16(VALUE self) {
  DEBUG_FUNCTION_ENTRY()
  compact_data* cd = get_cdata(self);   
  return INT2FIX(read_i16(cd));
}

static VALUE rb_read_i32(VALUE self) {
  DEBUG_FUNCTION_ENTRY()
  compact_data* cd = get_cdata(self);   
  return INT2NUM(zig_zag_to_int(read_varint64(cd)));
}

static VALUE rb_read_i64(VALUE self) {
  DEBUG_FUNCTION_ENTRY()
  compact_data* cd = get_cdata(self);   
  return LL2NUM(zig_zag_to_ll(read_varint64(cd)));
}
...
void Init_layered_compact_protocol() {
  ...
  // These are empty.
  // rb_define_method(dummy_class, "write_field_end", rb_thrift_compact_proto_write_field_end, 0);
  // rb_define_method(bpa_class, "write_map_end", rb_thrift_compact_proto_write_map_end, 0);
  // rb_define_method(dummy_class, "write_list_end", rb_thrift_compact_proto_write_list_end, 0);
  // rb_define_method(dummy_class, "write_set_end", rb_thrift_compact_proto_write_set_end, 0);
  ...
  //These are empty.
  // rb_define_method(dummy_class, "read_message_end", rb_thrift_compact_proto_read_message_end, 0);
  // rb_define_method(dummy_class, "read_field_end",     rb_thrift_compact_proto_read_field_end, 0);
