...
static VALUE rb_read_into_buffer(VALUE self, VALUE buffer_value, VALUE size_value) {
  ...
  if (size_value == Qnil) rb_raise(rb_eArgError, "size_value cannot be NULL");
  if (buffer_value == Qnil) rb_raise(rb_eArgError, "buffer value cannot be NULL");

  len = FIX2INT(size_value);
  buflen = RSTRING_LEN(buffer_value);
  buf = RSTRING_PTR(buffer_value);
