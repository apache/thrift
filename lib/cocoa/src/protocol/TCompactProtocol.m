...
-(void)writeMessageBeginWithName:(NSString *)name type:(int)messageType sequenceID:(int)sequenceID
{
    ...
    [self writeByteDirect:(uint8_t)((COMPACT_VERSION & COMPACT_VERSION_MASK) | ((((uint32_t)messageType) << COMPACT_TYPE_SHIFT_AMOUNT) & COMPACT_TYPE_MASK))];
...
-(void)writeFieldBeginWithName:(NSString*)name type:(int)fieldType fieldID:(int)fieldID
{
    ...
    {
        [self writeFieldBeginInternalWithName:name type:fieldType fieldID:fieldID typeOverride:0xFF];
    }
}

-(void)writeFieldBeginInternalWithName:(NSString *)name type:(int)fieldType fieldID:(int)fieldID typeOverride:(uint8_t)typeOverride
{
...
-(void)writeMapBeginWithKeyType:(int)keyType valueType:(int)valueType size:(int)size
{
    if (size == 0)
    {
...
-(void)writeBool:(BOOL)b
{
    if (boolFieldId != nil && boolFieldName != nil && boolFieldType != nil)
    {
        ...
        [self writeFieldBeginInternalWithName:boolFieldName type:[boolFieldType intValue] fieldID:[boolFieldId intValue] typeOverride:b ? TCType_BOOLEAN_TRUE : TCType_BOOLEAN_FALSE];
...
-(void)readMessageBeginReturningName:(NSString**)pname type:(int*)ptype sequenceID:(int*)psequenceID
{
    uint8_t protocolId = [self readByte];
    if (protocolId != COMPACT_PROTOCOL_ID)
    {
        @throw [TProtocolException exceptionWithName:@"TProtocolException" reason:[NSString stringWithFormat:@"Expected protocol id %X but got %X", COMPACT_PROTOCOL_ID, protocolId]];
    }
    ...
    {
        @throw [TProtocolException exceptionWithName:@"TProtocolException" reason:[NSString stringWithFormat:@"Expected version %d but got %d", COMPACT_VERSION, version]];
    }
...
-(void)readFieldBeginReturningName:(NSString**)pname type:(int*)pfieldType fieldID:(int*)pfieldID
{
...
-(void)readMapBeginReturningKeyType:(int*)pkeyType valueType:(int*)pvalueType size:(int*)psize
{
...
-(void)readListBeginReturningElementType:(int*)pelementType size:(int*)psize
{
...
-(NSString*)readString
{
    ...
    return [[[NSString alloc] initWithData:[self readBinary:length] encoding:NSUTF8StringEncoding] autorelease_stub];
}
...
-(int)getTTypeForCompactType:(uint8_t)type
{
    switch (type & 0x0f)
    {
        ...
        default:
            @throw [TProtocolException exceptionWithName:@"TProtocolException" reason:[NSString stringWithFormat:@"Don't know what type: %d", (uint8_t)(type & 0x0F)]];
    }
}
