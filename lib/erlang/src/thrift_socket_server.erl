...
parse_options([{handler, Handler} | Rest], State) when
    State#thrift_socket_server.handler == undefined, is_atom(Handler)
->
    ...
    ServiceModuleMap =
        case State#thrift_socket_server.service of
            undefined ->
                lists:foldl(
                    fun
                        ({ServiceName, Service}, Acc) when
                            is_list(ServiceName), is_tuple(Service)
                        ->
...
parse_options([{service, Service} | Rest], State) when
    State#thrift_socket_server.service == undefined, is_tuple(Service)
->
...
gen_tcp_listen(Port, Opts, State) ->
    case gen_tcp:listen(Port, Opts) of
        {ok, Listen} ->
            ...
            {ok,
                new_acceptor(State#thrift_socket_server{
                    listen = Listen,
                    port = ListenPort
                })};
        {error, Reason} ->
...
handle_info(
    ...
    case Reason of
        ...
        _ ->
            error_logger:error_report(
                {?MODULE, ?LINE, {child_error, Reason, erlang:get_stacktrace()}}
            )
    end,
    State1 = State#thrift_socket_server{max = Max + 1},
    State2 =
