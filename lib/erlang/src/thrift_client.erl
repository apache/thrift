...
new(Protocol, Service = {Module, ServiceName}) when
    is_atom(Module), is_atom(ServiceName)
->
    {ok, #tclient{
        protocol = Protocol,
        service = Service,
...
send_function_call(Client = #tclient{service = Service}, Function, Args) ->
    {Params, Reply} =
        try
            {
                get_function_info(Service, Function, params_type),
                get_function_info(Service, Function, reply_type)
            }
        catch
            error:badarg -> {no_function, 0}
        end,
    MsgType =
        case Reply of
...
-spec receive_function_result(#tclient{}, atom()) ->
    {#tclient{}, {ok, any()} | {error, any()}} | no_return().
...
handle_reply(
    ...
    ReplyStructDef =
        {struct, struct, [{0, undefined, ReplyType, undefined, undefined}] ++ ExceptionFields},
