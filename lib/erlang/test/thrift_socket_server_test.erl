...
parse_handler_options_test_() ->
    ...
    CorrectHandlerTestFunction = fun() ->
        ?assertMatch(
            {thrift_socket_server, _, _, _, _, _, _, _, _, _, _, _, _, _},
            thrift_socket_server:parse_options([{handler, CorrectServiceHandlerOptionList}])
        ),
        {thrift_socket_server, _, _, HandlerList, _, _, _, _, _, _, _, _, _, _} = thrift_socket_server:parse_options(
    ...
    [
        ...
        {"Parse the handler option as a non multiplexed service handler",
            ?_assertMatch(
                {thrift_socket_server, _, _, ?MODULE, _, _, _, _, _, _, _, _, _, _},
...
parse_service_options_test_() ->
    CorrectServiceModuleOptionList = [
        {"Service1", {?MODULE, 'Service1'}}, {"Service2", {?MODULE, 'Service2'}}
    ],
    WrongService2ModuleOptionList = [
        {"Service1", {?MODULE, s1}}, {"Service2", "thrift_service_module"}
    ],
    ...
    CorrectServiceModuleTestFunction = fun() ->
        ...
        {thrift_socket_server, _, ServiceModuleList, _, _, _, _, _, _, _, _, _, _, _} = thrift_socket_server:parse_options(
            [{service, CorrectServiceModuleOptionList}]
        ),
        lists:foreach(
            fun({ServiceName, Service}) ->
                ?assertMatch(
                    {ok, Service} when is_tuple(Service),
    ...
    [
        ...
        {"Try to parse the service option twice",
            ?_assertThrow(
                _,
                thrift_socket_server:parse_options([
                    {service, {?MODULE, s1}}, {service, CorrectServiceModuleOptionList}
                ])
            )},
        {"Parse a service module for a non multiplexed service",
            ?_assertMatch(
                {thrift_socket_server, _, {?MODULE, s1}, _, _, _, _, _, _, _, _, _, _, _},
                thrift_socket_server:parse_options([{service, {?MODULE, s1}}])
            )},
