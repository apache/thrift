 ...
 */
public class TAsyncSSLClientManager {
  private static final Logger LOGGER = LoggerFactory.getLogger(TAsyncSSLClientManager.class.getName());

  private final SelectThread selectThread;
  private final ConcurrentLinkedQueue<TAsyncSSLMethodCall> pendingCalls = new ConcurrentLinkedQueue<TAsyncSSLMethodCall>();

  public TAsyncSSLClientManager() throws IOException {
    this.selectThread = new SelectThread();
    selectThread.start();
  }

  public void call(TAsyncSSLMethodCall method) throws TException {
    if (!isRunning()) {
  ...
  private class SelectThread extends Thread {
    ...
    private final TreeSet<TAsyncSSLMethodCall> timeoutWatchSet = new TreeSet<TAsyncSSLMethodCall>(new TAsyncSSLMethodCallTimeoutComparator());
    ...
    private void transitionMethods() {
      try {
        ...
        while (keys.hasNext()) {
          ...
          TAsyncSSLMethodCall methodCall = (TAsyncSSLMethodCall)key.attachment();
    ...
    private void timeoutMethods() {
      Iterator<TAsyncSSLMethodCall> iterator = timeoutWatchSet.iterator();
      long currentTime = System.currentTimeMillis();
      while (iterator.hasNext()) {
        TAsyncSSLMethodCall methodCall = iterator.next();
    ...
    private void startPendingMethods() {
      TAsyncSSLMethodCall methodCall;
      while ((methodCall = pendingCalls.poll()) != null) {
        ...
        try {
          ...
          TAsyncSSLClient client = methodCall.getClient();
  ...
  private static class TAsyncSSLMethodCallTimeoutComparator implements Comparator<TAsyncSSLMethodCall>, Serializable {
    public int compare(TAsyncSSLMethodCall left, TAsyncSSLMethodCall right) {
