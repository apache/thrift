...
class TestExceptionDocRendering(unittest.TestCase):
    ...
    def test_java_uses_escaped_qualified_exception_names(self):
        ...
        self.assertIn("/**\n     * @throws com.main.$native", content)
        self.assertIn("@throws com.main.$native First line of the explanation", content)
        self.assertIn("second line uses a < b && c > d", content)

    def test_javame_uses_escaped_qualified_exception_names(self):
        content = self._generate("javame", "com/main/ExceptionDocTest.java")
        self.assertIn("@throws com.incl.IncludedExc Included exception", content)

    def test_py_uses_escaped_qualified_exception_names(self):
        content = self._generate("py", "main/ExceptionDocTest.py")
        self.assertIn("Raises:", content)
        self.assertIn("native: First line of the explanation", content)
        self.assertIn("second line uses a < b && c > d", content)
