...
void t_cpp_generator::generate_struct_reader(ofstream& out,
  ...
  out <<
    ...
    endl <<
    indent() << "xfer += iprot->readStructBegin(fname_tmp_);" << endl <<
    endl <<
  ...
  indent(out) <<
    ...
    indent(out) <<
      "xfer += iprot->readFieldBegin(fname_tmp_, ftype, fid);" << endl;
...
void t_cpp_generator::generate_struct_clear(std::ofstream& out, t_struct* tstruct) {
  ...
  if (has_optional) {
    out << indent() << "memset(&__isset, 0, sizeof(__isset));\n";
  }
  scope_down(out);
  out << endl;
}
