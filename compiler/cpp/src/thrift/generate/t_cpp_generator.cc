...
public:
  ...
  std::string function_call(t_function* tfunction,
                            std::string prefix = "",
...
void t_cpp_generator::generate_service_interface(t_service* tservice, string style) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_header_ << indent() << "virtual " << function_signature(*f_iter, style, "", true, false) << " = 0;" << endl;
    if (style.empty()) {
      f_header_ << indent() << "virtual " << function_signature(*f_iter, style, "", true, true) << " {" << endl;
      indent_up();
      // Generate a forwarding call
      // to the non-context function for backward compatibility!
      f_header_ << indent() << function_call(*f_iter, "", false) << ";" << endl;
      indent_down();
...
void t_cpp_generator::generate_process_function(t_service* tservice,
  ...
  if (style != "Cob") {
    ...
    generate_function_call(out, tfunction, "result.success", "iface_", "args.", true);
...
/**
 * Renders a function nested call for the same object
 *
 * @param tfunction Function definition
 * @param prefix Function name prefix
 * @param with_context Should forward callContext
 * @return String of rendered function definition
 */
std::string t_cpp_generator::function_call(t_function* tfunction,
                                           std::string prefix,
                                           bool with_context) {
  ...
  if (is_complex_type(ttype)) {
    return prefix + tfunction->get_name()
           + "(_return" + argument_list(arglist, true, false, true, with_context) + ")";
  } else {
    return (ttype->is_void() ? "" : "return ") + prefix + tfunction->get_name()
           + "(" + argument_list(arglist, true, false, false, with_context) + ")";
  }
}
  ...
  if (style == "") {
    if (is_complex_type(ttype)) {
      return "void " + prefix + tfunction->get_name() + "(" + type_name(ttype)
             ...
             + argument_list(arglist, name_params, true, true, with_context) + ")";
    } else {
      return type_name(ttype) + " " + prefix + tfunction->get_name() + "("
             + argument_list(arglist, name_params, true, false, with_context) + ")";
    }
    ...
    return "void " + prefix + tfunction->get_name() + "(::std::function<void" + cob_type + "> cob"
           + exn_cob + argument_list(arglist, name_params, true, true, false) + ")";
  } else {
    throw "UNKNOWN STYLE";
  }
}
...
string t_cpp_generator::argument_list(t_struct* tstruct,
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    result += (type_params ? type_name((*f_iter)->get_type(), false, true) + " " : "")
              + (name_params ? (*f_iter)->get_name() : "/* " + (*f_iter)->get_name() + " */");
  }
  if (with_context) {
    return result + (first ? "" : ", ")
           + (type_params ? "void* " : "")
           + (name_params ? "__ctx__" : "/*__ctx__*/");
  }
