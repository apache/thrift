...
public:
  ...
  void generate_throws_doc(std::stringstream& out, t_function* function) {
    for (auto* exception : function->get_xceptions()->get_members()) {
      if (!out.str().empty()) {
        out << '\n';
      }
      out << "@throws " << get_doc_type_name(exception->get_type());
  ...
  static std::string xml_encode(std::string contents) {
    contents = replace_all(contents, "&", "&amp;");
    contents = replace_all(contents, "<", "&lt;");
    return replace_all(contents, ">", "&gt;");
  }
};
