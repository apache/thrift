...
The recipes below build only the compiler. To build the C++ library as well, add `boost libevent openssl` to the `brew install` line. With CMake, replace `-DBUILD_LIBRARIES=OFF` with `-DWITH_JAVASCRIPT=OFF -DWITH_NODEJS=OFF -DWITH_JAVA=OFF -DWITH_PYTHON=OFF`: every library whose tools CMake finds is built by default, and the JavaScript and Node.js ones run `npm install` without checking for Node.js first. With Autotools, drop `--disable-libs` and pass `--without-<lang>` for each other language whose tools are installed; `./configure --help` lists them.
