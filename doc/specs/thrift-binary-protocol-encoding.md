...
3. Server sends a `Message` (type `Response` or `Exception`) to start the response.
...
`Oneway` (step 1 in the protocol exchange). Servers send responses with TMessages of type `Exception` or `Reply`.
...
Unfortunately there is a bug in the Java code generated by Apache Thrift 0.9.1 up to 0.9.3 (and maybe earlier). Any
thrown exception that is not defined in the service IDL will still be send (that is: unchecked exceptions from the
service and decode/encode exceptions). There is no correct way to handle this situation from the client perspective; you
either wait for a response or not, you can't do both. Luckily this has been fixed _after_ Apache Thrift 0.9.3
(THRIFT-3479). As `oneway` is only used from Thrift 0.9.3 this will only be a problem if your client _and_ server use
that version. More research is needed to make statements of combinations with non-java implementations.
...
Note that the field name is not encoded in the binary or compact protocol. This may be different for other protocols.
...
Responses are structs in which exactly 1 of the following fields are encoded:

* A field with name `success` and id `0`, used in case the method completed normally.
* An exception field, name and id are as defined in the `throws` clause in the IDL's service method definition.
* An exception field, name and id 
