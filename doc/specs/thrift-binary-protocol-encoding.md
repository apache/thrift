...
Strangely enough (in the Java code generated by Apache Thrift 0.9.1 up to 0.9.3), only responses of type `Response` are
skipped. Responses of type `Exception` are always send. There is no correct way to handle this situation from the client
perspective; you either wait for a response or not, you can't do both. Luckily this has been fixed _after_ Apache Thrift
0.9.3 (THRIFT-3479). My advice is to avoid oneway methods unless you know exactly what behavior your stack has.
...
* `vvvvvvvvvvvvvvv` is the version, an unsigned 15 bit number fixed to `1`. The leading bit must be `1`.
...
Because name length must be positive (therefor the first bit is always `0`), the first bit allows the receiver to see
