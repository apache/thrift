...
start(Args) ->
  ...
  DemoDict = maps:from_list([ {Key, Key-10} || Key <- lists:seq(0,10) ]),
  DemoSet = ordsets:from_list(lists:seq(-3,3)),
  ...
  {Client14, {ok, five}}         = thrift_client:call(Client13, testEnum, [five]),
