...
handle_function(testMap, {Map = #{}}) ->
    io:format("testMap: ~p~n", [Map]),
    {reply, Map};

handle_function(testStringMap, {Map = #{}}) ->
    io:format("testStringMap: ~p~n", [Map]),
    {reply, Map};

handle_function(testSet, {Set}) ->
    true = ordsets:is_set(Set),
    io:format("testSet: ~p~n", [ordsets:to_list(Set)]),
    {reply, Set};
...
handle_function(testEnum, {Enum}) when is_atom(Enum) ->
...
handle_function(testMapMap, {Hello}) ->
    ...
    PosList = maps:from_list([{I, I}   || I <- lists:seq(1, 4)]),
    NegList = maps:from_list([{-I, -I} || I <- lists:seq(1, 4)]),

    MapMap = #{4 => PosList, -4 => NegList},
    {reply, MapMap};
...
handle_function(testInsanity, {Insanity}) when is_record(Insanity, 'Insanity') ->
    ...
    FirstMap = #{two => Insanity, three => Insanity},

    SecondMap = #{six => Looney},

    Insane = #{1 => FirstMap, 2 => SecondMap},

    io:format("Return = ~p~n", [Insane]),

    {reply, Insane};

handle_function(testMulti, Args = {Arg0, Arg1, Arg2, Arg3, Arg4, Arg5})
