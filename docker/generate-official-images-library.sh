...
if [ "${ALLOW_DIRTY_OFFICIAL_IMAGES_MANIFEST:-0}" != "1" ] &&
	{ ! git -C "$repo_root" diff --quiet -- docker .github/workflows/docker.yml doc/ReleaseManagement.md ||
		! git -C "$repo_root" diff --cached --quiet -- docker .github/workflows/docker.yml doc/ReleaseManagement.md; }; then
